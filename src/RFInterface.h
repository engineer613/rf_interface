...
class RFInterface {
public:
    RFInterface(const char* rf_ip = "127.0.0.1", uint16_t rf_port = 18084);

    // Main update method like the original
    void update(const struct control_input &input);
...
private:
    ...
    void exchange_data(const struct control_input &input);
